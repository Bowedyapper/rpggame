namespace Game {
    int windowWidth;
    int windowHeight;
    Uint64 _timeNow = SDL_GetPerformanceCounter();
    ...
    bool createGame(int windowWidth, int windowHeight, const char *title);
 ...
 * @returns True if successful.
 */
bool Game::_createWindow(int width, int height) {
    windowWidth = width;
    windowHeight = height;
    ...
    window = SDL_CreateWindow(_windowTitle,
...
bool Game::createGame(int windowWidth, int windowHeight, const char *title) {

    if (renderer || window) {
        utils::debugLog("warning", "Game already created, ignoring any other calls to createGame");
        return false;
    } else {
        _init();
        _windowTitle = title;
        _createWindow(windowWidth, windowHeight);
        _createRenderer();
...
double Game::calculateDeltaTime() {
    _previousTime = _timeNow;
    _timeNow = SDL_GetPerformanceCounter();
    delta = (double)(_timeNow - _previousTime) / (double)SDL_GetPerformanceFrequency() * 1000.0f;
    return delta;
}
